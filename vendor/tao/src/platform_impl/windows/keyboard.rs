  ...
  pub(crate) fn process_message(
    ...
    match msg_kind {
      ...
      win32wm::WM_KEYDOWN | win32wm::WM_SYSKEYDOWN => {
        ...
        // Backport of tao#1215: the peek moved to the event loop, before any
        // lock is taken — PeekMessageW can re-enter the window proc, and doing
        // that while LAYOUT_CACHE is held deadlocked the main thread.
        let kbd_state = get_kbd_state();
        let event_info = {
