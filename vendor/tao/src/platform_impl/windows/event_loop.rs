...
/// Backport of tao#1215 (released in 0.36.0): `PeekMessageW` can dispatch
/// pending *sent* messages, re-entering the event handler while it runs.
/// Calling it with any input-state mutex held (LAYOUT_CACHE, window_state)
/// deadlocks the main thread. All peeks therefore happen up front, through
/// these helpers, before any lock is taken.
  ...
  let callback = || match msg {
    ...
    _ => {
      if msg == *DESTROY_MSG_ID {
        ...
        // Backport of tao#1264 (released in 0.36.0): updating the taskbar can
        // synchronously re-enter this window proc; holding `window_state`
        // across that call deadlocks the main thread (triggered by Explorer
        // restarts and resume from sleep).
